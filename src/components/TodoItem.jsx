...
export default function TodoItem({ item, index, setData }) {
  ...
  const deleteItem = async () => {
    const { data } = await axios.delete(`/api/deleteItem/${item._id}`);
    setData(data[0].taskList);
  };
    ...
    <li className="sm:my-2">
      <div className="flex items-center text-xl py-3">
        <input
          ...
          checked={item.status}
        />
        <span className="flex-1" style={{ textDecoration: item.status ? "line-through" : "none" }}>
          {item.task}
        </span>
        {item.status ? (
          <div className=" mx-2 cursor-pointer " onClick={() => deleteItem()}>
            <svg
